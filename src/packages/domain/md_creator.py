...
Typical usage example:
    class SomeClass:
        ...
        def some_method(
            ...
            context = self.md_creator.create(result, source_pdf_path)
"""
...
class MdCreator:
    ...
    def create(
        ...
        paragraphs = self.__get_paragraphs(analyze_result)
        tables = self.__get_markdown_tables(analyze_result)

        section_elements = self.__get_section_elements(analyze_result)
        for element in section_elements:
            if element.startswith('/paragraphs/'):
                ...
                all_markdown += self.__get_figure_summarize(idx, analyze_result, source_pdf_path)
